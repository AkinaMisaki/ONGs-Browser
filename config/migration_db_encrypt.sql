...
ALTER TABLE usuario
    ...
    ADD    COLUMN IF NOT EXISTS email_hash VARCHAR(64) NULL AFTER email;

-- 2. Índice no blind index (busca por email sem expor o valor cifrado)
CREATE INDEX IF NOT EXISTS idx_email_hash ON usuario (email_hash(64));
...
ALTER TABLE usuario_verificacao
    ...
    ADD    COLUMN IF NOT EXISTS telegram_id_hash VARCHAR(64) NULL AFTER telegram_id;

CREATE INDEX IF NOT EXISTS idx_telegram_id_hash ON usuario_verificacao (telegram_id_hash);
